...
export class RenderMarkdownWrapper {
	...
	async renderMarkdownWrapper(
		...
		containerEl.findAll('.internal-embed').forEach((el) => {
			const link = this.parseLink(el.getAttribute('src') as string);
	...
	private parseLink(src: string) {
		...
		const matched = (typeof src === 'string' &&
			src.match(linkComponentsRegex)) as RegExpMatchArray;
			...
			el.createEl(
				...
				(img) => {
					if (el.hasAttribute('width'))
						img.setAttribute(
							'width',
							el.getAttribute('width') as string,
						);
					else img.setAttribute('width', '100%');
					if (el.hasAttribute('alt'))
						img.setAttribute(
							'alt',
							el.getAttribute('alt') as string,
						);
					el.addEventListener(
						'click',
						(ev) =>
	...
	private async renderTransclude(
		...
		if (!cache || !cache.headings || !cache.blocks) return;
		...
		if (link.heading) {
			...
			const endAt =
				cache.headings
					...
					.find((h) => h.level <= heading.level)?.position?.start
					?.offset || text.length;
