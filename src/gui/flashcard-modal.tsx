import { App, Editor, Modal, Notice, Platform, setIcon } from 'obsidian';
...
export class FlashcardModal extends Modal {
	...
	renderDecksList(): void {
		this.mode = FlashcardModalMode.DecksList;
		const stats: DeckStats = this.reviewSequencer.getDeckStats(
			TopicPath.emptyPath,
		);
		this.titleEl.setText(t('DECKS'));
	...
	renderDeck(
		...
		const deckStats = this.reviewSequencer.getDeckStats(
			deck.getTopicPath(),
		);
	...
	displayCurrentCardInfoNotice() {
		const schedule = this.currentCard.scheduleInfo as CardScheduleInfo;
	...
	async doEditQuestionText(): Promise<void> {
		...
		// 		this.reviewSequencer.updateCurrentQuestionText(
		// 			modifiedCardText,
		// 		);
		// 	})
	...
	private showAnswer(): void {
		...
		const wrapper: RenderMarkdownWrapper = new RenderMarkdownWrapper(
			this.app,
			this.plugin,
			this.currentNote.filePath as string,
		);
	...
	private async handleNextCard(): Promise<void> {
		if (this.currentCard != null) await this.traverseCurrentCard();
		else this.renderDecksList();
	}

	private async traverseCurrentCard() {
		await this.app.workspace.openLinkText(
			this.currentNote.file.basename,
			this.currentNote.file.path as string,
		);
	...
	// 			this.formatQuestionContextText(
	// 				this.currentQuestion.questionContext,
	// 			),
	// 		);
	// }
	...
	// ) {
	// 	const schedule: CardScheduleInfo =
