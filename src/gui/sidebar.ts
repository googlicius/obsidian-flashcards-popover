...
export const REVIEW_QUEUE_VIEW_TYPE = 'review-queue-list-view-2';
...
export class ReviewQueueListView extends ItemView {
	...
	public redraw(): void {
		...
		for (const deckKey in this.plugin.reviewDecks) {
			...
			if (deck.newNotes.length > 0) {
				const newNotesFolderEl: HTMLElement =
					this.createRightPaneFolder(
						deckFolderEl,
						t('NEW'),
				...
				for (const newFile of deck.newNotes) {
					const fileIsOpen =
						activeFile && newFile.path === activeFile.path;
					if (fileIsOpen) {
			...
			if (deck.scheduledNotes.length > 0) {
				...
				for (const sNote of deck.scheduledNotes) {
					...
					const fileIsOpen =
						activeFile && sNote.note.path === activeFile.path;
					if (fileIsOpen) {
						deck.activeFolders.add(deck.deckName);
						deck.activeFolders.add(folderTitle);
						this.changeFolderIconToExpanded(
							schedFolderEl as HTMLElement,
						);
						this.changeFolderIconToExpanded(deckFolderEl);
					}
	...
	private createRightPaneFolder(
		...
		const folderTitleEl: HTMLDivElement =
	...
	private changeFolderIconToExpanded(folderEl: HTMLElement): void {
		const collapseIconEl = folderEl.find(
			'div.nav-folder-collapse-indicator',
		);
		(collapseIconEl.childNodes[0] as HTMLElement).style.transform = '';
	}
}
