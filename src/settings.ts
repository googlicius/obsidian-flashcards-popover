...
export class SRSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const header = containerEl.createEl('h1', {
		...
		new Setting(containerEl)
			...
			.addTextArea((text) =>
				text
					.setValue(
						this.plugin.data.settings.noteFoldersToIgnore.join(
							'\n',
						),
					)
					.onChange((value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.data.settings.convertHighlightsToClozes,
					)
					.onChange(async (value) => {
						this.plugin.data.settings.convertHighlightsToClozes =
							value;
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						this.plugin.data.settings.convertBoldTextToClozes =
							value;
						await this.plugin.savePluginData();
					}),
			);
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.data.settings.enableNoteReviewPaneOnStartup,
					)
					.onChange(async (value) => {
						this.plugin.data.settings.enableNoteReviewPaneOnStartup =
							value;
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.data.settings
							.initiallyExpandAllSubdecksInTree,
					)
					.onChange(async (value) => {
						this.plugin.data.settings.initiallyExpandAllSubdecksInTree =
							value;
						await this.plugin.savePluginData();
					}),
			);
		...
		containerEl.createDiv().innerHTML = t('CHECK_ALGORITHM_WIKI', {
			algo_url:
				'https://www.stephenmwangi.com/obsidian-spaced-repetition/algorithms/',
		});
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(this.plugin.data.settings.showDebugMessages)
					.onChange(async (value) => {
